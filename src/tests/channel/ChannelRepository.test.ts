...
describe("initializeDatabase", () => {
  ...
  it("should work with default client", () => {
    const { channelRepository } = initializeDatabase(mockLogger);
    expect(PrismaClient).toHaveBeenCalledTimes(1);
    expect(channelRepository).toBeInstanceOf(ChannelRepository);
  });

  it("should work with injected client", () => {
    const mockClient = new PrismaClient();
    const prismaService = new PrismaService(mockClient);
    const { channelRepository } = initializeDatabase(mockLogger, prismaService);
    expect(channelRepository).toBeInstanceOf(ChannelRepository);
  });
});
