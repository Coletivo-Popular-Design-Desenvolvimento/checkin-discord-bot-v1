...
export default class CreateChannel implements ICreateChannelUseCase {
    ...
    async executeAsync(channel: CreateChannelInput): Promise<GenericOutputDto<ChannelEntity>> {
        const existingChannel = await this.channelRepository.findByDiscordIdAsync(channel.name);
        if (existingChannel) {
            return {
                data: null,
                success: false,
