...
import { enviaEmail } from "./bot/email";
import { readEvents } from "./bot/file";
import { enviaMensagem } from "./bot/telegram";
  ...
  dados.then(function (result) {
    for (let key in result) {
      ...
      let dataEvento = new Date(
        `${dataSeparada[1]}/${dataSeparada[2]}/${dataSeparada[0]}`
      ); //Cria o objeto date com a data do evento
      ...
      if (
        dataEvento.toString() <= data30Mmn.toString() &&
        dataEvento.toString() >= dataAtual.toString()
      ) {
        //Primeiro verifica se o evento ocorre em 30min ou menos
        console.log(
          key,
          "Disparo de mensagem 30min antes.",
          dataEvento.toString()
        );
        enviaEmail();
        enviaMensagem();
      } else if (
        dataEvento.toString() <= data1dia.toString() &&
        dataEvento.toString() >= dataAtual.toString()
      ) {
        //Depois verifica se o evento ocorre em 24h ou menos
        console.log(
          key,
          "Disparo de mensagem 1 dia antes.",
          dataEvento.toString()
        );
        enviaEmail();
        enviaMensagem();
